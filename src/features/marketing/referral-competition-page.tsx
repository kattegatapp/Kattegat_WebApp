        ...
        <section className="relative isolate overflow-hidden border-b border-brand-forest/8 px-5 py-16 sm:px-8 lg:py-24">
          ...
          <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1.08fr_.92fr] lg:items-center">
            <div>
              ...
              <h1 className="mt-6 text-5xl font-extrabold leading-[1.02] tracking-[-0.04em] sm:text-6xl lg:text-7xl">Refer. Climb.<br /><span className="bg-gradient-to-r from-[#C6F3CA] via-brand-mantis to-brand-emerald bg-clip-text text-transparent">Win AED {competition.prizePoolAed.toLocaleString()}.</span></h1>
              ...
              <p className="mt-4 text-xs text-brand-forest/50">Founding Members are eligible. Kattegat employees are not. Only qualified active referrals count.</p>
              <div className="mt-10 flex items-end gap-2">
                {prizes.map((prize) => <div key={prize.place} className={`flex min-h-36 flex-1 flex-col items-center justify-center rounded-3xl border p-4 text-center shadow-[0_16px_40px_rgb(0_57_18/0.08)] backdrop-blur-xl ${prize.place === 1 ? "min-h-44 border-[#C9A24B]/55 bg-gradient-to-b from-[#fffaf0] to-white" : "border-white/90 bg-white/65"}`}>{prize.place === 1 ? <Crown className="mb-2 size-6 text-[#C9A24B]" /> : null}<span className={`text-[10px] font-extrabold uppercase tracking-[0.2em] ${prize.place === 1 ? "text-[#9A762A]" : "text-brand-forest/55"}`}>{prize.place === 1 ? "1st" : prize.place === 2 ? "2nd" : "3rd"}</span><strong className="mt-1 text-base text-brand-forest sm:text-xl">AED {prize.amountAed.toLocaleString()}</strong><span className="mt-1 text-[10px] text-brand-forest/50">{prize.minimumActiveReferrals}+ active referrals</span></div>)}
              </div>
            </div>
