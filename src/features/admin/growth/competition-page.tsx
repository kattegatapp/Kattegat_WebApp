  ...
  return <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
    ...
    <div className="grid gap-5 lg:grid-cols-[1fr_.8fr]">
      <Card><CardHeader><CardTitle>Competition controls</CardTitle></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2">
        ...
        {[1,2,3].map((place) => <div key={place} className="rounded-xl border p-4 sm:col-span-2"><p className="font-extrabold">{place === 1 ? "First" : place === 2 ? "Second" : "Third"} place</p><div className="mt-3 grid grid-cols-2 gap-3"><label className="text-xs font-bold">Prize AED<Input type="number" value={form[`${place === 1 ? "first" : place === 2 ? "second" : "third"}PrizeAed`] ?? ""} onChange={(e) => field(`${place === 1 ? "first" : place === 2 ? "second" : "third"}PrizeAed`, e.target.value)} /></label><label className="text-xs font-bold">Active referrals<Input type="number" value={form[`${place === 1 ? "first" : place === 2 ? "second" : "third"}Threshold`] ?? ""} onChange={(e) => field(`${place === 1 ? "first" : place === 2 ? "second" : "third"}Threshold`, e.target.value)} /></label></div></div>)}
