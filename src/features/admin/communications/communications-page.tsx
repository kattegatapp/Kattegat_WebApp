...
function ChannelRow({ icon: Icon, label, description, checked, disabled, onChange }: { icon: typeof Mail; label: string; description: string; checked: boolean; disabled?: boolean; onChange: (value: boolean) => void }) { return <div className={cn("flex items-start gap-3 rounded-2xl border p-4 sm:items-center", checked ? "border-emerald-200 bg-emerald-50/60" : "border-border/70", disabled && "opacity-50")}><span className={cn("mt-0.5 flex size-10 shrink-0 items-center justify-center rounded-xl sm:mt-0", checked ? "bg-brand-forest text-white" : "bg-muted text-muted-foreground")}><Icon className="size-5" /></span><div className="min-w-0 flex-1"><p className="text-sm font-semibold text-brand-forest">{label}</p><p className="text-xs leading-5 text-muted-foreground">{description}</p></div><Switch className="mt-1 shrink-0 sm:mt-0" checked={checked} disabled={disabled} onCheckedChange={onChange} /></div>; }
